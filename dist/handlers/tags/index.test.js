...
describe("Mentors tests", () => {
  ...
  const table = [];
  models.tags.get.mockImplementation(async params => {
    const {
      query,
      limit,
      offset
  ...
  models.tags.list.mockImplementation(async params => {
    const {
      query,
      limit = 10,
      offset = 0
  ...
  models.tags.insert.mockImplementation(async row => {
  ...
  models.tags.update.mockImplementation(async row => {
    const {
      id
    } = row;
  ...
  models.tags.remove.mockImplementation(async row => {
    const {
      id
